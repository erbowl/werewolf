$(function () {
	...
	function Night(room, peer) {
		...
		if (dead_peer) {
			...
			if (dead_peer == peer.id) {
				...
				$("video#" + dead_peer).get(0).muted = true;
	...
	function dead_mode() {
		...
		$("#dead").show();
		$("video").show();
		$("video.other-peer").each(function (i) {
